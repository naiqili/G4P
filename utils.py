...
def get_res(paras, phase, maxiter, subn, lb):
    ...
    if mattype == -1: # pearson        
        ...
        data_path1 = path='./tmp2/%s_phase%02d_iter%d_subn%d_lb%d_M%d_HATS_matcnt%d_inv%s_ggd%s.npz'%  \
        (paras['dataset'], phase, paras['maxiter'], paras['subn'], paras['lb'], paras['M'], paras['matcnt'], paras['inverse'], paras['ggd'])
    print(data_path1)
    data1 = np.load(data_path1)
    if 'val_predY0' not in data1:
        return data1['predY0'], data1['std_varY0'], data1['sample_Y0']
    else:
        return data1['predY0'], data1['std_varY0'], data1['sample_Y0'], data1['val_predY0'], data1['val_std_varY0'], data1['val_sample_Y0']
